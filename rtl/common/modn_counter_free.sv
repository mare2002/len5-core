...
module modn_counter_free #(
  ...
  parameter logic [$clog2(N):0] INIT = N  // initial value at reset and clear
) (
  // Input signals
  input logic clk_i,
  ...
  logic [$clog2(N):0] count_q;
  logic carry;
  
  assign adder_res1 = count_q + {{$clog2(N)-I-1{1'b0}},increase_val_i};
  
  assign {carry, adder_res2} = adder_res1 - {{$clog2(N)-D-1{1'b0}},decrease_val_i};
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      count_q <= INIT;  // Asynchronous reset
    end else if (clr_i) begin
      count_q <= INIT;  // Synchronous clear when requested
  ...
  assign empty_no = (count_q == N[$clog2(N):0]);
endmodule
